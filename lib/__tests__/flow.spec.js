...
describe('pipeP', () => {
	...
	it('arrays cannot be passed as first argument', () => {
		return expect(() =>
			pipeP([])()
		).rejects.toThrow('firstFunc.apply is not a function');
	});

	it('this argument is propagated to argument functions', async() => {
		const thisArg = {};
...
describe('parallelMerge', () => {
	...
	it('catch handles errors in argument functions', async() => {
		const result = await parallelMerge(() => {
			throw Error('Something went wrong')
		})().catch((error) => {
			expect(error.message).toBe('Something went wrong');
